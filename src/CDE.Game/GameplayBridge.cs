...
internal sealed class GameplayBridge
{
    ...
    public readonly CDE.Gameplay.Kernel.FlagStore Flags = new();
    private CDE.Gameplay.Kernel.WarpKernel? _warp;
    private CDE.Gameplay.Kernel.KernelWorld? _mario;
    ...
    public void LoadFromRepoRoot(string repoRoot)
    {
        ...
        if (File.Exists(yumePath))
        {
            ...
            var g = CDE.Gameplay.Kernel.WarpKernel.LoadWarpGraphJson(json);
            _warp = new CDE.Gameplay.Kernel.WarpKernel(g, Flags);
        }

        if (File.Exists(marioPath))
        {
            var json = File.ReadAllText(marioPath, new UTF8Encoding(false));
            _mario = CDE.Gameplay.Kernel.KernelWorld.LoadMarioJson(json);
        }
    }
    ...
    public void Tick()
    {
        if (ActiveMode == Mode.Yume)
        {
            ...
            var r = _warp.TryWarp(new CDE.Gameplay.Kernel.WarpKernel.WarpRequest(SceneId, X, Y));
            if (r.Warped)
            {
        ...
        else
        {
            ...
            var p = new CDE.Gameplay.Kernel.KernelWorld.PlayerState(SceneId, X, Y);
            var r = _mario.Tick(p);
