...
function Write-Utf8NoBomLf([string]$path,[string]$text){
  ...
  $b = $u.GetBytes($text.Replace("`r`n","`n"))
...
if($LASTEXITCODE -ne 0){ Die "DOTNET_BUILD_FAILED_AFTER_MARIO_KERNEL_FIX_V2" }
Write-Host "CDE_MARIO_KERNEL_FIX_V2_BUILD_OK" -ForegroundColor Green
& dotnet run --project (Join-Path $RepoRootAbs "src\CDE.Gameplay.Demo\CDE.Gameplay.Demo.csproj") -c Debug | Out-Host
if($LASTEXITCODE -ne 0){ Die "DEMO_RUN_FAILED_AFTER_MARIO_KERNEL_FIX_V2" }
Write-Host "CDE_MARIO_KERNEL_FIX_V2_DEMO_OK" -ForegroundColor Green
