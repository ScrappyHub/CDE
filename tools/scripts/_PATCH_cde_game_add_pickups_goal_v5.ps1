...
$t = (ReadUtf8 $GameRoot).Replace("`r`n","`n")
...
if($t -notmatch "CDE_GAME_PICKUPS_INIT_V5"){
  ...
  $t2 = $t.Substring(0,$idx) + "`n`n" + $Init + "`n" + $t.Substring($idx)
  if($t2 -eq $t){ Die "FAILED_INSERT_INIT" }
  $t = $t2
}

if($t -notmatch "CDE_GAME_PICKUPS_UPDATE_V5"){
...
if($t -notmatch "CDE_GAME_PICKUPS_DRAW_V5"){
...
Write-Host ("PATCH_OK: pickups+goal v5 applied: " + $GameRoot) -ForegroundColor Green
