...
# 1) Add MonoGame PackageReference to CDE.Runtime.csproj
...
$needle = ("PackageReference Include=""{0}""" -f $Pkg)
if($rt -match [regex]::Escape($needle)){
  ...
  if(-not $rt2.EndsWith("`n")){ $rt2 = $rt2 + "`n" }
  Write-Utf8NoBomLf $RuntimeCsproj $rt2
  Write-Host ("PATCH_OK: runtime now references MonoGame (backup=" + $bak + "): " + $RuntimeCsproj) -ForegroundColor Green
}
...
# 3) Tilemap: stub type + alias it in PlatformerController
...
if(Test-Path -LiteralPath $Plat -PathType Leaf){
  ...
  if(-not $has){
    ...
    Write-Host ("PATCH_OK: added Tilemap alias (backup=" + $bak + "): " + $Plat) -ForegroundColor Green
  } else {
