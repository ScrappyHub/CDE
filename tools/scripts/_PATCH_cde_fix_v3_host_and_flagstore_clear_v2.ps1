...
# --- FIX #1: runner v3 "$Host" collision (case-insensitive) ---
Backup-IfExists $Runner
$rt = [System.IO.File]::ReadAllText($Runner,[System.Text.Encoding]::UTF8)
$c = 0
# replace: $host = ReadAllTextUtf8 $HostPath   ->   $hostText = ReadAllTextUtf8 $HostPath
$rt2 = [System.Text.RegularExpressions.Regex]::Replace($rt, '(?m)^\s*\$host\s*=\s*ReadAllTextUtf8\s+\$HostPath\s*$', '$hostText = ReadAllTextUtf8 $HostPath')
if($rt2 -ne $rt){ $rt = $rt2; $c++ }
# replace uses of $host in the specific marker check
$rt2 = [System.Text.RegularExpressions.Regex]::Replace($rt, '(?m)\bif\s*\(\s*\$host\s+-notmatch\b', 'if($hostText -notmatch')
if($rt2 -ne $rt){ $rt = $rt2; $c++ }
if($c -gt 0){ Write-Utf8NoBomLf $Runner $rt; Write-Host ("PATCH_OK: runner v3 host var fix changes=" + $c) -ForegroundColor Green } else { Write-Host "SKIP: runner v3 host var already OK" -ForegroundColor Yellow }

# --- FIX #2: FlagStore has no Clear() ---
...
for($i=0; $i -lt $lines.Length; $i++){
  $ln = $lines[$i]
  if([System.Text.RegularExpressions.Regex]::IsMatch($ln, '^\s*Flags\.Clear\(\)\s*;\s*$')){ $removed++; continue }
  [void]$out.Add($ln)
}
if($removed -gt 0){ $newText = (@($out.ToArray()) -join "`n") + "`n"; Write-Utf8NoBomLf $Bridge $newText; Write-Host ("PATCH_OK: removed Flags.Clear() removed=" + $removed) -ForegroundColor Green } else { Write-Host "SKIP: no Flags.Clear() found" -ForegroundColor Yellow }
